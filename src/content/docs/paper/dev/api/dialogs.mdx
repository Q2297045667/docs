...
public class ServerJoinListener implements Listener {
    ...
    void onPlayerConfigure(AsyncPlayerConnectionConfigureEvent event) {
        ...
        if (!response.join()) {
            // 我们手动关闭对话框，因为客户端可能不会自行关闭它。
            audience.closeDialog();
            // 如果响应为 false，则表示他们拒绝了。因此，我们将他们从服务器中踢出。
            connection.disconnect(Component.text("你讨厌 Paper-chan ：(", NamedTextColor.RED));
        }
